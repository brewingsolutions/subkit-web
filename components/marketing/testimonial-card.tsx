...
export function TestimonialCard({ testimonial, className, ...props }: TestimonialCardProps) {
  return (
    <div
      className={cn(
        "relative rounded-2xl border bg-card p-8 transition-all duration-500 flex flex-col justify-between hover:-translate-y-1 shadow-[0_8px_30px_rgb(0,0,0,0.02)] group",
        testimonial.color,
        className
      )}
      {...props}
